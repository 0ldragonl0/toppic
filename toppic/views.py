...
def UserProfile(request):
    now = datetime.datetime.now()
    now = formats.date_format(now,"SHORT_DATETIME_FORMAT")
    return render(request, 'userprofile.html',{'users':User.objects.filter(username =request.user),"date":str(now)})
...
def Device(request):
    ...
    if request.POST.get('deletedevice'):
         ...
         DeviceProfile.objects.filter(id__in=request.POST.getlist('items')).delete()
            ...
            return render(request, '/updatedevice/', {'form': form,"data":data,"date":str(now)})
